...
CREATE TABLE dossier_countries (
  article_id UUID REFERENCES articles(id) ON DELETE CASCADE,
  country_code CHAR(2) REFERENCES countries(code),
  PRIMARY KEY (article_id, country_code)
);

CREATE TABLE dossier_indicators (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE dossier_timeline_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  article_id UUID NOT NULL REFERENCES articles(id) ON DELETE CASCADE,
...
CREATE TABLE dossier_legislation (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  article_id UUID NOT NULL REFERENCES articles(id) ON DELETE CASCADE,
...
CREATE INDEX dossier_indicators_article_idx ON dossier_indicators (article_id, display_order, observed_on);
CREATE INDEX dossier_timeline_article_idx ON dossier_timeline_events (article_id, event_date, display_order);
CREATE INDEX dossier_legislation_article_idx ON dossier_legislation (article_id, display_order, enacted_on);
